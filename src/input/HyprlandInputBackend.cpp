...
HyprlandInputBackend::HyprlandInputBackend(void *handle)
    ...
    , m_keyboardKeyHook(handle, "onKeyboardKey")
...
{
    m_eventListeners.push_back(Event::bus()->m_events.input.touch.down.listen([this](const ITouch::SDownEvent &event, Event::SCallbackInfo &info) {
        touchDown(info, event);
    }));
...
void HyprlandInputBackend::touchDown(Event::SCallbackInfo &info, const ITouch::SDownEvent &event)
{
    if (m_ignoreEvents) {
        return;
    }
...
void HyprlandInputBackend::keyboardKeyHook(void *thisPtr, const IKeyboard::SKeyEvent &event, SP<IKeyboard> sender)
{
    ...
    if (!self->keyboardKey(self->findInputActionsDevice(sender.get()), event.keycode, event.state == WL_KEYBOARD_KEY_STATE_PRESSED)) {
        self->m_keyboardKeyHook(thisPtr, event, sender);
    }
}
